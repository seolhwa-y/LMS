...
<script type="text/javascript">
	...
	function init(){
		...
		shippingareavar = new Vue({
			el: '#divShippingList',
			components:{	'divComGrpCodList' : divComGrpCodList	},
			data:{
				listitem: [],
				action: ""
			},
			methods:{
				detailview: function(no){
					alert(no);
					shippingModalOpen(no);
				}
			}
		});
		
		shippingeditvue = new Vue({
			el:'#deliDirection',
			data:{
				dir_code:"",
				JORD_DATE:"",
					...
					<div class="content">
						...
						<div class="divComGrpCodList">
							<table class="col" id="shippingDirectionTable">
								...
								<tbody id="shippingDirectionList" v-for="(item,index) in listitem" v-if="listitem.length">
									<tr @click="detailview(item,JORD_CODE)">
										<th>{{item.JORD_CODE}}</th>
										<th>{{item.PD_NAME}}</th>
										<th>{{item.JORD_DATE}}</th>
										<th>{{item.PD_CORP}}</th>
										<th>{{item.JORD_AMT}}</th>
										<th>{{item.JORD_WISHDATE}}</th>
										<th>{{item.SH_TYPE}}</th>
									</tr>
								</tbody>
							</table>
		...
		<dl>
			...
			<dd class="content">
				...
				<table class="col" id="refundDiretionTable">
					...
					<tr>
							<td><a type="text" class="inputTxt p100" name="DIR_CODE" id="JORD_DATE" v-model="JORD_DATE" v-bind:readonly="JORD_DATEreadonly" /></a></td>
							<td id="JORD_CODE" v-model="JORD_CODE"></td>
							<td id="PD_CORP" v-model="PD_CORP"></td>
							<td id="PD_NAME" v-model="PD_NAME"></td>
							<td id="JORD_AMT" v-model="JORD_AMT"></td>
							<td id="WH_NAME" v-model="WH_NAME"></td>
							<td id="name" v-model="name"></td>
							<td id="JORD_IN" v-model="JORD_IN"></td>
						</tr>
