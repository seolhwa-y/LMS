	 				  ...
	 				  var listCallBack = function(data) {
						  console.log(data);
	 					  ...
	 					  for(i = 0; i < data.length; i++){
		 					  ...
		 					  vueOrderReturnDetail.disabled[i] = true;
	 					  }
	 					  console.log(vueOrderReturnDetail.disabled);
	 				  }
		...
		vueOrderReturnDetail = new Vue({
			...
			methods : {
				...
				checkAction : function (action){
					...
					switch(action) {
					case 're' : 
						vueOrderReturnDetail.action = 'return'; 
						vueOrderReturnDetail.returnShow = true;
						vueOrderReturnDetail.rePle = false;
						vueOrderReturnDetail.stPle = false;
						vueOrderReturnDetail.disabled = false;
						break;
					case 'st' : 
						vueOrderReturnDetail.action = 'stock'; 
						vueOrderReturnDetail.stockShow = true;
						vueOrderReturnDetail.rePle = false;
						vueOrderReturnDetail.stPle = false;
						vueOrderReturnDetail.disabled = false;
						break;
				...
				insertOrderReturn : function(){ console.log("insertOrderReturn");
					...
					console.log(param);
	 				...
	 				var listCallBack = function(data) {
						 ...
						 vueOrderReturnDetail.disabled = true;
						 vueOrderReturnDetail.rePle = false;
						 vueOrderReturnDetail.stPle = false;
						 vueOrderReturnDetail.returnShow = false;
						 vueOrderReturnDetail.stockShow = false;
	 				}
						...
						<div id="divOrderReturnDetailList">
							<table class="col">
								...
								<tbody>
									<tr v-for = " (item, index) in orderReturnDetailList" v-if = "orderReturnDetailList.length > 0" >
										<td>
											<input type="checkBox" name = "isCheck" v-bind:disabled = "disabled" /> <!-- 반품처리 및 재고처리에 필요한 체크박스 -->
										</td>
							...
							<div class = "paging_area"  id = "divPurDirectionPaging" v-html = "pagenavi"></div>
                           	...
                           	<div style="margin-left: 40%;">
                             	...
                             	<a class="btnType gray" v-on:click = "insertOrderReturn()" v-show = "returnShow"><span>반품처리</span></a>
                           		<a class="btnType blue" v-on:click = "updateOrderReturn()" v-show = "stockShow"><span>재고처리</span></a> 
                           	</div>
						</div>
