...
<script type="text/javascript">
	...
	function init() {
		...
		oHistoryList = new Vue({
			...
			methods: {
				bDirection:function(no){
					oDetailModal(no);
				}
			}
		});
	
		detailBorder = new Vue({
			el:"#detailBorder",
			data : {
				listItem : []
			}
								...
								<table class="col">
									...
									<tbody id = "orderHistoryTBody" v-for="(item, index) in listItem" v-if="listItem.length" >
										<tr>
											...
											<a href="" v-else id='btnBordDirec' class='btnType blue' name="btn"><span>작성</span></a>
											</td>
										   
										    <!-- 배송지시서  -->
										    <td>
										    <a v-if="item.shCode != 0" > 작성완료  </a>
											<a href=""  v-else id='btnShipDirec' class='btnType blue' name="btn"><span>작성</span></a>
											</td>
										</tr>   
									</tbody>
								</table>
